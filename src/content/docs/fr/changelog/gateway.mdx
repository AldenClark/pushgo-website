...
### Amélioré
- Fiabilité améliorée du rafraîchissement des identifiants fournisseur : les chemins de nouvelle tentative ciblés forcent une récupération fraîche auprès du token-provider, tandis que l'envoi normal reste prioritaire sur le cache.
- Sélection explicite du profil d'exécution : `small` reste le profil par défaut pour les déploiements privés ou légers, et `public` cible les fortes charges.
- Les réglages fins des files, du dispatch, des fournisseurs, des pools de base de données et de SQLite sont désormais des valeurs internes au profil afin de réduire les risques de mauvaise configuration.
- Les lignes de statistiques conservées après échec de persistance sont bornées, ce qui évite une croissance mémoire non limitée en cas de pression temporaire sur le stockage.
